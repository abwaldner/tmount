  ...
  реализованы через использование su или sudo.  Поэтому "tmount" по
  умолчанию запускает их без установки таймаута.  Будьте осторожны: в
  таком случае зависание этих скриптов блокирует работу приложения.  По
  этой же причине контейнеры никогда не разблокируются автоматически.
