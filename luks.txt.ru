  ...
  cryptsetup, если получат право на её выполнение без ввода пароля.
  Впрочем, при этом не потребуется никакая модификация скриптов.  Кроме
  того, при использовании эмуляторов терминала или утилит наподобие
  ktsuss (http://github.com/nomius/ktsuss, графическая версия su)
  теряются код возврата и содержимое stderr вызываемой программы.
  Поэтому использование этих вариантов не рекомендуется.  Для работы с
  su предпочтительней использовать expect-скрипт.
      В настоящее время все скрипты разблокировки принимают
  дополнительный параметр, определяющий способ ввода пароля LUKS:
  "-k" - запросить имя файла ключа, "-i" - интерактивный ввод, "-a" или
  отсутствие параметра - диалог выбора.
  ...
  Замечание: программа pumount работает в данном случае некорректно,
  ...
  zenity) и пр. возможно, но значительно менее удобно.  Вы можете, в
  частности, использовать 'zenity 2>/dev/null' вместо 'qarma'.
