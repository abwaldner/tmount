...
v. 0.0.9 -
  1) Для действий "Oтключить" и "Извлечь" добавлена проверка на loopback-
     отображения.
...
v. 0.0.8 - 2018.09.13
  ...
  4) Скрипты lock-crypsetup-* пытаются отмотировать все точки
     монтирования ФС контейнера, если необходимо.  Скрипты
     unlock-cryptsetup-* вызывают команду монтирования, если в
     контейнере найдена ФС.  Во всех скриптах исправлена обработка имён
     файлов и точек монтирования, содержащих не-ascii и непечатаемые
     символы.
  5) Улучшен порядок вывода элементов меню.
  6) Изменено поведение действий "Извлечь" и "Отключить".  Добавлено
     действие "Отмонтировать все точки монтирования".
