...
v. 0.0.10 - 2022.05.13
  1) Для смонтированных файловых систем теперь показывается ёмкость ФС вместо
     ёмкости носителя.  Ёмкость носителя для этих ФС видна, если установлена
     опция "Показывать подробнее".
  2) Добавлен показ меток разделов APT(Mac) и GPT.
  3) Добавлено распознавание разделов Atari TOS.
  4) Улучшен показ типов разделов GPT.
  5) Вследствие удаления пакета Qt4 из gentoo  это последний выпуск, который
     может быть скомпилирован с его использованием.
