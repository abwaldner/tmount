      ...
      Александр Б. Вальднер, 2016-2022.
  ...
  устройству.
      ...
      Для монтирования/размонтирования tmount по умолчанию использует udevil
  (https://ignorantguru.github.com/udevil).  Вы можете использовать любую
  подходящую программу или собственный скрипт путём редактирования полей
  "Монтировать" и "Отмонтировать" в окне диалога "Настройки".  Имя файла
  устройства (напр. /dev/sdc1 или /dev/sr0) будет добавлено в конец заданной
  командной строки для команд монтирования и разблокирования контейнера, и
  файловое имя точки монтирования будет добавлено для команды отмонтирования.
  Например, просто введите "pmount" и "pumount" в соответствующие поля для
  использования программы pmount (http://pmount.alioth.debian.org).  Имя файла
  устройства полного диска (напр. /dev/sdc для /dev/sdc[1-9] и /dev/sr0 для
  /dev/sr0), будет добавлено для команд извлечения носителя и отключения
  устройства.  Если вы оставите поля "Извлечь" и/или "Отключить" пустыми,
  соответствующие пункты меню просто станут недоступны.
  ...
  запись "./bin/myscrypt".
      Для каждого действия в соответствующем поле задаётся таймаут в секундах,
  0 - без таймаута.
